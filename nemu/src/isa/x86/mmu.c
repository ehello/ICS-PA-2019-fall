...
// +----------------+----------------+---------------------+
  ...
  uint32_t pg_tab = paddr_read(cpu.cr3.val + pg_dir_idx * sizeof(PDE), sizeof(PDE));//不知道为什么不用pg_dir_base
  //检查page table第一个entry的present位
  assert(pg_tab & 0x1);

  uint32_t pg_frame = paddr_read((pg_tab & 0xfffff000) + pg_tab_idx * sizeof(PTE), sizeof(PTE));
